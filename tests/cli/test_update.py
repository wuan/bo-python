...
import blitzortung.cli.update as update
...
@pytest.fixture
def config():
    with patch('blitzortung.cli.update.blitzortung.config.config') as mock:
        yield mock


@pytest.fixture
def fetch():
    with patch('blitzortung.cli.update.fetch_strikes_from_url') as mock:
        yield mock


@pytest.fixture
def strike_db():
    with patch('blitzortung.cli.update.blitzortung.db.strike') as mock:
...
class TestFetchStrikesFromUrl:
    ...
    def test_fetch_strikes_successfully(self, mock_response):
        ...
        with patch('blitzortung.cli.update.requests.get', return_value=mock_response):
            strikes = list(update.fetch_strikes_from_url('http://example.com/strikes'))
        ...
        assert_that(first_strike.x).is_equal_to(134.5895)
        assert_that(first_strike.y).is_equal_to(-15.2966)
    ...
    def test_fetch_handles_empty_lines(self):
        ...
        assert_that(strikes[0].x).is_equal_to(134.5895)
        assert_that(strikes[0].y).is_equal_to(-15.2966)
    ...
    def test_fetch_raises_on_http_error(self):
        ...
        with patch('blitzortung.cli.update.requests.get', side_effect=requests.RequestException("Connection error")):
            with pytest.raises(requests.RequestException):
                list(update.fetch_strikes_from_url('http://example.com/strikes'))

    def test_fetch_with_authentication(self, mock_response):
        """Test fetch with authentication credentials."""
        with patch('blitzortung.cli.update.requests.get', return_value=mock_response) as mock_get:
            list(update.fetch_strikes_from_url('http://example.com/strikes', auth=('user', 'pass')))
...
class TestCreateStrikeKey:
    ...
    def test_create_strike_key_with_timestamp_value(self):
        ...
        key = update.create_strike_key(strike)
    ...
    def test_create_strike_key_rounds_location(self):
        ...
        strike = create_strike(datetime.datetime.now(datetime.timezone.utc), 12.34567890123, 45.67890123456)
...
def create_strike(timestamp: datetime.datetime | int, x: float, y: float, lateral_error: int = 0) -> Strike:
    global strike_id
    strike_id += 1
    return Strike(strike_id, Timestamp(timestamp), x, y, 0, 0, lateral_error, [])


class TestUpdateStrikes:
    """Integration tests for update_strikes function."""

    def test_update_strikes_inserts_new_strikes(self, config, fetch, strike_db):
        """Test that new strikes are inserted."""

        # Create strikes from URL
        delayed = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=1)
        strike1 = create_strike(delayed, 10.5, 20.5)
        strike2 = create_strike(delayed, 11.5, 21.5)

        fetch.return_value = [strike1, strike2]

        # Run update
        result = update.update_strikes(hours=1)
    ...
    def test_update_strikes_skips_current_strikes(self, config, fetch, strike_db):
        """Test that new strikes are inserted."""

        # Create strikes from URL
        delayed = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(seconds=59)
        strike1 = create_strike(delayed, 10.5, 20.5)

        fetch.return_value = [strike1]
    ...
    def test_update_strikes_skips_duplicates(self, config, fetch, strike_db):
        """Test that existing strikes are not re-inserted."""
        # Use same timestamp and location for both strikes
        delayed = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=30)
        ...
        fetch.return_value = [strike_from_url]
    ...
    def test_update_strikes_filters_by_time_interval(self, config, fetch, strike_db):
        ...
        # One strike within interval (30 minutes ago), one outside (2 hours ago)
        strike_in_interval = create_strike(now - datetime.timedelta(minutes=30), 10.5, 20.5)
        strike_outside_interval = create_strike(now - datetime.timedelta(hours=2), 11.5, 21.5)

        fetch.return_value = [strike_in_interval, strike_outside_interval]
    ...
    def test_failure_at_fetch(self, config, fetch, strike_db):
        ...
        fetch.side_effect = requests.RequestException("Connection error")
    ...
    def test_failure_at_insert(self, config, fetch, strike_db):
        """Test that database errors are properly handled."""

        # Create a strike within the time interval
        delayed = datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(minutes=30)
        strike = create_strike(delayed, 10.5, 20.5)
