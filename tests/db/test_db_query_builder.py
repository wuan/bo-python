...
class StrikeTest(object):
    def setUp(self):
        ...
        self.end_time = datetime.datetime.now(datetime.timezone.utc).replace(
    ...
    def test_select_query(self):
        query = self.query_builder.select_query(
            "<table_name>",
            self.srid,
    ...
    def test_grid_query(self):
        ...
        parameters = query.get_parameters()
        assert_that(parameters.keys()).contains(
            "xmin",
            "ymin",
            "xdiv",
            "ydiv",
...
class StrikeClusterTest(object):
    ...
    def test_select_query(self):
        ...
        parameters = query.get_parameters()
        assert_that(parameters.keys()).contains(
            "timestamps", "srid", "interval_seconds"
        )
        assert_that(tuple(parameters["timestamps"])).is_equal_to(
            tuple(str(self.end_time - self.interval_duration * i) for i in range(0, 6))
        )
        assert_that(parameters["srid"]).is_equal_to(self.srid)
        assert_that(parameters["interval_seconds"]).is_equal_to(
            self.interval_duration.total_seconds()
        )
