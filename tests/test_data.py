...
class TestEvent(EventBaseTest):
    def test_create_and_get_values(self):
        ...
        assert_that(event.timestamp, is_(self.now_time))
        assert_that(event.x, is_(11))
        assert_that(event.y, is_(49))
...
class TestStrike(unittest.TestCase):
    ...
    def test_get_id(self):
        assert_that(self.strike.id, is_(equal_to(123)))

    def test_get_timestamp(self):
        assert_that(self.strike.timestamp, is_(equal_to(self.timestamp)))

    def test_get_location(self):
        assert_that(self.strike.x, is_(equal_to(11.2)))
        assert_that(self.strike.y, is_(equal_to(49.3)))

    def test_get_altitude(self):
        assert_that(self.strike.altitude, is_(equal_to(2500)))

    def test_get_amplitude(self):
        assert_that(self.strike.amplitude, is_(equal_to(10.5)))

    def test_get_lateral_error(self):
        assert_that(self.strike.lateral_error, is_(equal_to(5400)))

    def test_get_station_count(self):
        assert_that(self.strike.station_count, is_(equal_to(11)))

    def test_get_default_stations(self):
        self.strike = blitzortung.data.Strike(123, self.timestamp, 11.2, 49.3, 2500, 10.5, 5400, 11)
        assert_that(self.strike.stations, is_(equal_to([])))

    def test_with_stations(self):
        assert_that(self.strike.stations, is_(equal_to([1, 5, 7, 15])))
...
class TestStrikeCluster(unittest.TestCase):
    ...
    def test_get_id(self):
        assert_that(self.strike_cluster.id, is_(self.id_value))

    def test_get_timestamp(self):
        assert_that(self.strike_cluster.timestamp, is_(self.timestamp))

    def test_get_seconds_interval(self):
        assert_that(self.strike_cluster.interval_seconds, is_(self.seconds_interval))

    def test_get_shape(self):
        assert_that(self.strike_cluster.shape, is_(self.shape))

    def test_get_strike_count(self):
        assert_that(self.strike_cluster.strike_count, is_(self.strike_count))

    def test_get_area(self):
        assert_that(self.strike_cluster.area, is_(self.area))
...
class TestStation(unittest.TestCase):
    ...
    def test_get_number(self):
        assert_that(self.station.number, is_(123))

    def test_get_user(self):
        assert_that(self.station.user, is_(45))

    def test_get_name(self):
        assert_that(self.station.name, is_('<name>'))

    def test_get_country(self):
        assert_that(self.station.country, is_('<country>'))

    def test_get_status(self):
        assert_that(self.station.status, is_('<status>'))
...
class TestGridData(unittest.TestCase):
    ...
    def test_get_grid(self):
        assert_that(self.grid_data.grid, is_(self.grid))
