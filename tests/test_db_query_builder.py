...
class StrikeClusterTest(unittest.TestCase):
    ...
    def test_select_query(self):
        ...
        assert_that(str(query), is_(
            "SELECT id, \"timestamp\", stroke_count, ST_Transform(geog::geometry, %(srid)s) FROM <table_name> WHERE \"timestamp\" in (%(timestamps)s) AND interval_seconds=%(interval_seconds)s"))
        parameters = query.get_parameters()
