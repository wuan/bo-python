...
from blitzortung_srv.lock import LockWithTimeout, FailedToAcquireException
...
def import_strikes(regions, start_time):
    ...
    for region in regions:
        for retry in range(5):
            try:
                with stopit.SignalTimeout(300) as timeout_ctx:
                    import_strikes_for(region, start_time)
                break
            except (requests.exceptions.ConnectionError, stopit.TimeoutException):
                logger.info('import failed: retry {} region {}'.format(retry, region))
                error_count += 1
                time.sleep(2)
                continue
...
def import_station_info_for(region):
    ...
    if current_stations:
        ...
        offline_limit_time = offline_limit_time.replace(tzinfo=datetime.timezone.utc)
...
def run():
    ...
    try:
        with lock.locked(10):
            ...
            import_strikes(regions=regions, start_time=start_time)
