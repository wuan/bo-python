...
def prepare_grid_if_applicable(options, area) -> blitzortung.geom.Grid | None:
...
def main():
    ...
    if grid:
        fetch_strikes_grid(grid, options, strike_db, time_interval)
    else:
        # set query parameters
        order = 'timestamp'

        fetch_strikes(area, options, order, strike_db, time_interval)
...
def fetch_strikes(area: BaseGeometry | None, options: Values, order: str, strike_db, time_interval: TimeInterval):
