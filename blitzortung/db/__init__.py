...
class Query(object):
    ...
    def add_order(self, order):
        self.order_entries.append(order if isinstance(order, Order) else Order(order))
    ...
    def __str__(self):
        ...
        if self.order_entries:
            order_query_elements =\
                [order.get_column() + (' DESC' if order.is_desc() else '') for order in self.order_entries]
