...
class Strike:
    ...
    def grid_query(table_name, grid, count_threshold=0, **kwargs):
        return (
            GridQuery(grid, count_threshold)
            .set_table_name(table_name)
            .set_default_conditions(**kwargs)
        )
    ...
    def global_grid_query(table_name, grid, count_threshold=0, **kwargs):
        return (
            GlobalGridQuery(grid, count_threshold)
            .set_table_name(table_name)
            .set_default_conditions(**kwargs)
        )
    ...
    @staticmethod
    def histogram_query(
        table_name, minutes, minute_offset, binsize, region=None, envelope=None
    ):
        query = (
            SelectQuery()
            .set_table_name(table_name)
            .add_column(
                "-extract(epoch from clock_timestamp() + interval '%(offset)s minutes'"
                ' - "timestamp")::int/60/%(binsize)s as interval'
            )
            .add_column("count(*)")
            .add_condition(
                "\"timestamp\" >= (select clock_timestamp() + interval '%(offset)s minutes'"
        ...
        if envelope and envelope.env.is_valid:
            query.add_condition(
                "ST_SetSRID(CAST(%(envelope)s AS geometry), %(envelope_srid)s) && geog",
...
class StrikeCluster:
    def select_query(
        ...
        interval_offset = (
            interval_duration
            if interval_offset is None or interval_offset.total_seconds() <= 0
            else interval_offset
        )
        start_time = (
            timestamp - interval_offset * (interval_count - 1) - interval_duration
        )
        ...
        query = (
            ...
            .add_parameters(srid=srid)
            .add_condition(
                '"timestamp" in %(timestamps)s',
                timestamps=tuple(
                    str(timestamp)
                    for timestamp in self.get_timestamps(
                        start_time, end_time, interval_duration, interval_offset
                    )
                ),
            )
            .add_condition(
