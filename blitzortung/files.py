...
class Raw:
    ...
    def __execute(self, start_time, end_time, *additional_args):
        ...
        if end_time:
            args += ["-e", end_time]
        data_pipe = subprocess.Popen(
            args + list(additional_args), stdout=subprocess.PIPE
        )
...
class Archive:
    def __init__(self, config):
        ...
        for current_path, dirs, files in os.walk(self.root_path):
            ...
            if depth == 3:
                date_string = "-".join(
                    self.__split_path_into_components(current_path)[-depth:]
                )
    ...
    def get_files_for_date(self, date_string):
        ...
        if date in self.dates_filecount:
            for file_path in glob.glob(
                os.path.join(self.__get_path_for_date(date), "*")
            ):
