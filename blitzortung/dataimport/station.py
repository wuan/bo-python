...
class StationsBlitzortungDataProvider:
    ...
    def get_stations(self, region=1):
        ...
        target_url = self.data_url.build_path(
            "Protected/stations.txt.gz", region=region
        )
        for station_line in self.data_transport.read_lines(
            target_url, post_process=self.pre_process
        ):
            try:
                current_stations.append(
                    self.station_builder.from_line(station_line).build()
                )
