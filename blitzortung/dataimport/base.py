...
class HttpFileTransport(FileTransport):
    ...
    def read_lines(self, source_url, post_process=None):
        ...
        if response.status_code != 200:
            self.logger.debug(
                "http status %d for get '%s" % (response.status_code, source_url)
            )
            return []

        return self.split_lines(
            post_process(response.content).splitlines()
            if post_process
...
class BlitzortungDataPath:
    ...
    def __init__(self, base_path=None):
        self.data_path = os.path.join(
            (base_path if base_path else "https://{host_name}.blitzortung.org"), "Data"
        )
...
class BlitzortungDataPathGenerator:
    ...
    def get_paths(self, start_time, end_time=None):
        for interval_start_time in util.time_intervals(
            start_time, self.time_granularity, end_time
        ):
