...
class StrikeGridQuery:
    ...
    def create(
        ...
        state = StrikeGridState(
            statsd_client, grid_parameters, grid_metadata, time_interval
        )

        query = self.strike_query_builder.grid_query(
    ...
    def build_strikes_grid_result(results, state):
        state.add_info_text(
            "grid query %.03fs #%d %s"
            % (state.get_seconds(), len(results), state.grid_parameters)
        )
        ...
        strikes_grid_result = tuple(
            ...
            if 0 <= result["rx"] < x_bin_count and 0 < result["ry"] <= y_bin_count
        )
    ...
    def combine_result(self, strike_grid_result, histogram_result, state):
        combined_result = gatherResults(
            [strike_grid_result, histogram_result], consumeErrors=True
        )
    ...
    def build_grid_response(results, state):
        ...
        state.log_gauge(
            f"strikes_grid.size.{state.grid_metadata.base_length}", len(grid_data)
        )
        if state.grid_metadata.region is None:
            state.log_gauge(
                "local_strikes_grid.size", len(grid_data) if grid_data else 0
            )
            state.log_gauge(
                f"local_strikes_grid.size.{state.grid_metadata.base_length}",
                len(grid_data),
            )
...
class GlobalStrikeGridQuery:
    ...
    def create(
        ...
        query = self.strike_query_builder.global_grid_query(
            db.table.Strike.table_name,
            grid_parameters,
            time_interval=time_interval,
            count_threshold=count_threshold,
    ...
    def build_grid_response(results, state):
        ...
        state.log_gauge(
            f"global_strikes_grid.size.{state.grid_metadata.base_length}",
            len(grid_data),
        )
