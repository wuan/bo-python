...
class StrikeQuery:
    ...
    def __init__(
        self,
        strike_query_builder: db.query_builder.Strike,
        strike_mapper: db.mapper.Strike,
    ):
        self.strike_query_builder = strike_query_builder
        self.strike_mapper = strike_mapper

    def create(
        self, id_or_offset, minute_length, minute_offset, connection, statsd_client
    ):
        ...
        query = self.strike_query_builder.select_query(
            db.table.Strike.table_name,
            geom.Geometry.default_srid,
    ...
    def strike_build_results(self, query_result, state):
        state.add_info_text(
            "query %.03fs #%d" % (state.get_seconds(), len(query_result))
        )
