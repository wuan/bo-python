...
class Geometry:
    ...
    def __init__(self, srid: int = default_srid):
        self.srid: int = srid
    ...
    @property
    @abstractmethod
    def env(self) -> shapely.geometry.LinearRing:
        pass
...
class Envelope(Geometry):
    ...
    __slots__: tuple[str, str, str, str] = "x_min", "x_max", "y_min", "y_max"
...
class GridFactory:
    ...
    def __init__(
        self, min_lon, max_lon, min_lat, max_lat, coord_sys, ref_lon=None, ref_lat=None
    ):
    ...
    def get_for(self, base_length):
        if base_length not in self.grid_data:
            ref_lon = (
                self.ref_lon if self.ref_lon else (self.min_lon + self.max_lon) / 2.0
            )
            ref_lat = (
                self.ref_lat if self.ref_lat else (self.min_lat + self.max_lat) / 2.0
            )

            utm_x, utm_y = pyproj.Transformer.from_crs(
                self.WGS84, self.coord_sys
            ).transform(ref_lat, ref_lon)
            lat_d, lon_d = pyproj.Transformer.from_crs(
                self.coord_sys, self.WGS84
            ...
            self.grid_data[base_length] = Grid(
                self.min_lon,
                max_lon,
                self.min_lat,
                max_lat,
